...
SELECT `Long Name`, `Region`
FROM wdi_country
WHERE 'Long Name' = 'State of Qatar'
""")
...
WHERE (r.`Region`, i.`Income Group`) NOT IN (
...
SELECT c.`Region`, c.`Income Group`, c.count,
...
SELECT `Region`,
  ROUND(100.0 * SUM(CASE WHEN `Income Group` = 'High income' THEN count ELSE 0 END) / t.total_count, 2) AS High,
  ROUND(100.0 * SUM(CASE WHEN `Income Group` = 'Upper middle income' THEN count ELSE 0 END) / t.total_count, 2) AS Upper_Middle,
  ROUND(100.0 * SUM(CASE WHEN `Income Group` = 'Lower middle income' THEN count ELSE 0 END) / t.total_count, 2) AS Lower_Middle,
  ROUND(100.0 * SUM(CASE WHEN `Income Group` = 'Low income' THEN count ELSE 0 END) / t.total_count, 2) AS Low,
  ROUND(100.0 * SUM(CASE WHEN `Income Group` IS NULL OR `Income Group` = '' THEN count ELSE 0 END) / t.total_count, 2) AS Other
FROM base, totals t
...
SELECT 
  ...
  t.total_countries,
  ROUND(100.0 * COUNT(*) / t.total_countries, 1) AS percent_of_total
FROM 
  wdi_country, total t
GROUP BY 
  `Income Group`, t.total_countries
ORDER BY 
  country_count DESC;
""")
